  ...
  <div ref="root">
    ...
    >
      <router-link :to="getPath(data)" tag="div">
        <template v-if="data[config.icon] && data[config.icon].indexOf('iconfont-') !== -1">
          <svg class="icon" aria-hidden="true">
            <use :xlink:href="`${data[config.icon].replace('iconfont-', '')}`"></use>
          </svg>{{ data[config.text] }}
        </template>
        <template v-else>
          <i :class="data[config.icon]"></i>{{ data[config.text] }}
        </template>
        <i
